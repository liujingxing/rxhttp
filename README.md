...
Gradle引用方法
    ...
    dependencies {
       implementation 'com.http.wrapper:httpsender:1.0.7'
...
Get请求
        ...
        Disposable disposable = HttpSender
                ...
                .compose(LifecycleObserver.get(this)) //加入感知生命周期的观察者
                .subscribe(address -> {
...
Generated API Post请求

        String url = "http://ip.taobao.com/service/getIpInfo.php";
        Disposable disposable = Params.postForm(url) //发送Form表单形式的Post请求
                .add("ip", "63.223.108.42")//添加参数
                .addHeader("accept", "*/*") //添加请求头
                .addHeader("connection", "Keep-Alive")
                .addHeader("user-agent", "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1;SV1)");
...
Generated API 上传文件进度监听
        ...
        Disposable disposable = Params.postForm(url) //发送Form表单形式的Post请求
                ...
                .add("key2", "value2")//添加参数，非必须
                .addHeader("versionCode", "100"); //添加请求头,非必须
                .upload(new SimpleParser<String>() {}) //注:如果需要监听上传进度，使用upload操作符
                .observeOn(AndroidSchedulers.mainThread()) //主线程回调
                .doOnNext(progress -> {
                    //上传进度回调,0-100，仅在进度有更新时才会回调
                    int currentProgress = progress.getProgress(); //当前进度 0-100
